...
void window_status_update(Vis *vis, Win *win)
{
	...
	sb_push_str8(&status, filename.length > 0 ? filename : str8("[No Name]"));
	...
	if (vis->input_queue.length) {
		sb_push(&right, vis->input_queue.data, MIN(vis->input_queue.length, S32_MAX));
