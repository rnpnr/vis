...
ui_term_backend_blit(Ui *ui)
{
	...
	Cell *fb     = ui->vt100.cell_buffer.cells;
	s32   width  = ui->width;
		...
		for (s32 x = 0; x < width; x++) {
			if (!vis_cell_equal(fb, bb)) {
				...
				if (style.attributes != attributes) {
					...
					for (u64 i = 0; i < countof(cell_attrs); i++) {
						...
						if ((attributes & flag) != (style.attributes & flag)) {
							buffer_appendf(buf, "\x1b[%sm", (style.attributes & flag) ?
							               cell_attrs[i].on :
							               cell_attrs[i].off);
						}
					}
				...
				if (!vis_terminal_style_equal(fg, style_fg)) {
					if (style_fg.indexed) {
						buffer_appendf(buf, "\x1b[38;5;%um", (u32)style_fg.color.index);
					} else {
						buffer_appendf(buf, "\x1b[38;2;%u;%u;%um", (u32)style_fg.color.rgb.r,
				...
				if (!vis_terminal_style_equal(bg, style_bg)) {
					if (style_bg.indexed) {
						buffer_appendf(buf, "\x1b[48;5;%um", (u32)style_bg.color.index);
					} else {
						buffer_appendf(buf, "\x1b[48;2;%u;%u;%um", (u32)style_bg.color.rgb.r,
						               (u32)style_bg.color.rgb.g, (u32)style_bg.color.rgb.b);
					}
					bg = style_bg;
				}

				buffer_append0(buf, bb->data);
	...
	buffer_appendf(buf, "\x1b[%d;%dH", ui->cur_row + 1, ui->cur_col + 1);
	vis_ui_vt100_output((str8){.data = (u8 *)buf->data, .length = buffer_length0(buf)});
}

VIS_INTERNAL void ui_term_backend_clear(Ui *tui) { }
VIS_INTERNAL void ui_term_backend_save(Ui *tui, bool fscr) {}
VIS_INTERNAL void ui_term_backend_restore(Ui *tui) {}
