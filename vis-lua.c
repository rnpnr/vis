...
static int complete_command(lua_State *L) {
	...
	File *file = vis->win ? vis->win->file : 0;
	Filerange range = text_range_new(0, 0);

	static char cmd_list[VIS_COMMAND_BUFFER_MAX];
	static char cmd[VIS_COMMAND_BUFFER_MAX + 32]; /* 32 for echo/grep/vis-menu */
	cmd_list[0] = 0;
	cmd[0]      = 0;
	...
	int status = vis_pipe_collect(vis, file, &range, (const char *[]){cmd, 0}, &out, &err, false);
